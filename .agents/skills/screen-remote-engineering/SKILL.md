...
- During the `Screen-Remote/` pre-push workflow, analyze only the committed outgoing app range and the locked latest dadb commit. Ignore uncommitted app and dadb changes and do not compare dadb against a remote branch. Use dadb only as dependency evidence for wiki accuracy; do not place unrelated dadb-only work in the app commit message. Do not rewrite history or perform Git writes outside the wiki.
