...
if (!existsSync(path.join(appRepo, ".git"))) throw new Error("Screen-Remote repository not found");
for (const hookName of hookNames) {
  const hook = path.join(hooks, hookName);
  if (!existsSync(hook)) throw new Error(`${hookName} hook not found`);
