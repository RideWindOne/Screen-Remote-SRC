    ...
    suspend fun pairWithCode(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                LogManager.d(LogTags.ADB_PAIRING, "Starting Wireless Debugging pairing via dadb runtime: $ipAddress:$port")
                LogManager.d(LogTags.ADB_PAIRING, "Pairing identity: ${AdbRuntimeDiagnostics.identitySummary(adbRuntime)}")
                LogManager.d(LogTags.ADB_PAIRING, "Pairing TLS state before request: ${AdbRuntimeDiagnostics.endpointSummary(context, ipAddress, port)}")
                ...
                if (pairingResult.isSuccess) {
                    ...
                    LogManager.d(LogTags.ADB_PAIRING, "Pairing TLS state after request: ${AdbRuntimeDiagnostics.endpointSummary(context, ipAddress, port)}")
                    Result.success(Unit)
                } else {
                    val error = pairingResult.exceptionOrNull() ?: IllegalStateException("Wireless Debugging pairing failed")
