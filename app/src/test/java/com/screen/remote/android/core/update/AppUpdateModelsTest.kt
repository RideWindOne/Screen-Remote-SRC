...
class AppUpdateModelsTest {
    ...
    fun fourPartCurrentVersionEqualsReleaseTag() {
        assertNull(
            selectLatestRelease(
                listOf(GitHubReleaseInfo("4.4.3.5", "4.4.3.5", "url", false, false)),
                "4.4.3.5",
    ...
    fun apkAssetMatchesDeviceAbiBeforeUniversalFallback() {
        val arm64 = GitHubReleaseAsset("Screen.Remote-arm64-v8a-4.4.3.3.apk", "arm64-url", 10, null)
        val x86 = GitHubReleaseAsset("Screen.Remote-x86_64-4.4.3.3.apk", "x86-url", 10, null)
        val universal = GitHubReleaseAsset("Screen.Remote-universal-4.4.3.3.apk", "universal-url", 20, null)
    ...
    fun stableChannelIgnoresPrereleases() {
        val releases =
            listOf(
                GitHubReleaseInfo("1.1.0-beta.1", "beta", "beta-url", prerelease = true, draft = false),
                GitHubReleaseInfo("1.0.1", "stable", "stable-url", prerelease = false, draft = false),
            )

        assertEquals(
            "1.0.1",
            selectLatestRelease(releases, "1.0.0", UpdateChannel.STABLE)?.tagName,
        )
    }
    ...
    fun prereleaseChannelAcceptsPrereleases() {
        ...
        assertEquals(
            "1.1.0-beta.1",
            selectLatestRelease(releases, "1.0.0", UpdateChannel.PRERELEASE)?.tagName,
        )
    }
    ...
    fun draftsAndOlderVersionsAreIgnored() {
        val releases =
            listOf(
                GitHubReleaseInfo("2.0.0", "draft", "draft-url", prerelease = false, draft = true),
                GitHubReleaseInfo("0.9.9", "old", "old-url", prerelease = false, draft = false),
            )

        assertNull(selectLatestRelease(releases, "1.0.0", UpdateChannel.PRERELEASE))
    }
}
